      ...
      try {
        ...
        if (gitStatus.stdout.trim().length > 0) {
          ...
          if (confirmStash) {
            ...
            const stashResult = await pi.exec(
              "git",
              ["stash", "save", `pi-auto-stash-${Date.now()}`],
              { timeout: 10000 }
            );
            if (stashResult.code === 0) {
              ctx.ui.notify(
                "Changes stashed successfully. Proceeding with file modification.",
                "success"
              );
            } else {
              ctx.ui.notify(
                `Failed to stash changes: ${stashResult.stderr || stashResult.stdout}`,
                "error"
              );
              ...
              if (!proceedAnyway) {
                return {
                  block: true,
                  reason: "User chose not to proceed after failed stash.",
                };
              }
            }
          } else {
            ...
            if (!proceedAnyway) {
              return {
                block: true,
                reason: "User chose not to proceed with uncommitted changes.",
              };
            }
          }
        }
        ...
        ctx.ui.notify(
          `Git check failed (Is Git installed and repository initialized?): ${error.message}`,
          "warning"
        );
      }
    }
  ...
  pi.registerCommand("scaffold", {
    ...
    handler: async (args: string, ctx) => {
      ...
      if (!validTypes.includes(type)) {
        ctx.ui.notify(
          `Invalid scaffold type: ${type}. Valid types are: ${validTypes.join(", ")}.`,
          "error"
        );
        return;
      }
    ...
    handler: async (args: string, ctx) => {
      ...
      try {
        ...
        if (result.code === 0) {
          ...
          ctx.ui.notify(
            `Jest tests failed${filePath ? ` for ${filePath}` : ""}!\n${truncatedOutput}`,
            "error"
          );
...
`;
  ...
  pi.registerTool({
    ...
    description:
      "Validates a Suno music prompt based on basic guidelines (minimum length, common musical keywords).",
    parameters: Type.Object({
      prompt: Type.String({
        description: "The Suno music prompt string to validate.",
      }),
    }),
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (prompt.length < minLength) {
        validationMessages.push(
          `Prompt is too short (${prompt.length} chars). Minimum length is ${minLength} characters.`
        );
        isValid = false;
      }
      ...
      if (!foundKeywords) {
        validationMessages.push(
          `Prompt does not seem to contain common musical keywords. Consider including terms like: ${keywords.slice(0, 5).join(", ")}...`
        );
        isValid = false;
      }
      ...
      for (const word in wordCounts) {
        if (wordCounts[word] > maxRepetition && words.length > 10) {
          // Only check for longer prompts
          validationMessages.push(
            `Word "${word}" is highly repetitive (${wordCounts[word]} times). Avoid excessive repetition.`
          );
          isValid = false;
          break;
        }
