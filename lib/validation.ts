...
export function validatePromptConfig(config: unknown): config is PromptConfig {
    ...
    if (c.negativePrompt !== undefined && (typeof c.negativePrompt !== 'string' || c.negativePrompt.length > MAX_LONG_TEXT_LENGTH)) return false;
...
export function validateMutateRequest(data: unknown): data is { prompt: string; type: MutationType } {
    ...
    if (!d.prompt || typeof d.prompt !== 'string' || d.prompt.length > MAX_LONG_TEXT_LENGTH || d.prompt.trim().length === 0) {
        return false;
    }
