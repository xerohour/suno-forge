...
// Sort keys by length descending to ensure longest match first in regex
const MOOD_KEYS = Object.keys(MOOD_MAP).sort((a, b) => b.length - a.length).join('|');
const MOOD_REGEX = new RegExp(`\\b(${MOOD_KEYS})\\b`, 'gi');

const VOCAL_REMOVE_REGEX = /\b(vocal|vocals|singing|lyrics|voice|sung)\b[^,]*/gi;
const CLEANUP_COMMA_REGEX_1 = /,\s*,+/g;
const CLEANUP_COMMA_REGEX_2 = /^,\s*|\s*,$/g;
const TEMPO_MATCH_REGEX = /(\d+)\s*BPM/i;
...
const MUTATION_HANDLERS: Record<MutationType, (p: string) => string> = {
  ...
  instrumental: (p) => {
    ...
    let result = p.replace(VOCAL_REMOVE_REGEX, '');
    // Clean up multiple commas and extra spaces
    result = result.replace(CLEANUP_COMMA_REGEX_1, ',').replace(CLEANUP_COMMA_REGEX_2, '').trim();
    return `${result}, instrumental only, no vocals`;
  },

  'tempo-shift-up': (p) => {
    // Increase tempo references
    const tempoMatch = p.match(TEMPO_MATCH_REGEX);
    if (tempoMatch) {
  ...
  'mood-invert': (p) => {
    // Invert mood descriptors using single-pass regex replacement
    // This is O(N) compared to the previous O(N*M) approach
    return p.replace(MOOD_REGEX, (match) => {
      return MOOD_MAP[match.toLowerCase()] || match;
    });
  },
