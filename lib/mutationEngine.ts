...
// Pre-compile regex for single-pass replacement
// Optimization: Pre-compiling regex avoids recompilation on every call
const MOOD_REGEX = new RegExp(`\\b(${Object.keys(MOOD_MAP).join('|')})\\b`, 'gi');

// Optimization: Static handlers avoid object recreation on every call
const MUTATION_HANDLERS: Record<MutationType, (p: string) => string> = {
  ...
  'mood-invert': (p) => {
    // Invert mood descriptors using single-pass regex to avoid circular replacements
    // Optimization: Replaced multi-pass placeholder logic with single regex pass (O(N*M) -> O(N))
