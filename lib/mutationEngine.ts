...
// Pre-compile regex to avoid O(N*M) passes and circular replacement issues
const MOOD_REGEX = new RegExp(`\\b(${Object.keys(MOOD_MAP).join('|')})\\b`, 'gi');

// Define handlers statically to avoid object reconstruction on every call
const MUTATION_HANDLERS: Record<MutationType, (p: string) => string> = {
  ...
  'mood-invert': (p) => {
    ...
    return p.replace(MOOD_REGEX, (match) => MOOD_MAP[match.toLowerCase()] || match);
  },
