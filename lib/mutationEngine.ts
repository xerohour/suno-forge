...
// Security: Escape dictionary keys to prevent potential Regex injection vulnerabilities
// even if current static keys do not contain special characters.
const escapedMoodKeys = Object.keys(MOOD_MAP).map(key => key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
