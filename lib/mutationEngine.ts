...
// Pre-compute mood map and regex for O(1) lookup and O(N) replacement
...
// Create a regex that matches all keys in MOOD_MAP
...
const MUTATION_HANDLERS: Record<MutationType, (p: string) => string> = {
  ...
  'mood-invert': (p) => {
    // Single-pass replacement using callback function
    // This naturally handles circular replacements (e.g. happy -> melancholic -> joyful)
    // because the regex consumes the matched string and doesn't re-scan the replacement.
