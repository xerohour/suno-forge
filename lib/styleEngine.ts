...
export function buildStyle(config: PromptDNA): string {
  ...
  for (const p of rawParts) {
    if (p) {
      ...
      // Optimization: Use .includes() for deduplication instead of Array.from(new Set()).
      // O(N^2) operation is used here because the array is strictly bounded (max ~9 items),
      // making it faster by avoiding Set object allocation overhead.
