...
export function buildStyle(config: PromptDNA): string {
  ...
  // O(N^2) deduplication via .includes() is used here instead of Array.from(new Set())
  // because the array is strictly bounded (max ~9 items) and this avoids Set allocation overhead.
