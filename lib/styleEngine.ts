...
export function buildStyle(config: PromptDNA): string {
  ...
  for (const p of rawParts) {
    if (p) {
      ...
      // O(N^2) `.includes()` lookup is used here instead of Array.from(new Set())
      // because the array is strictly bounded (max 9 items) and avoiding the
      // object allocation overhead of Set provides a measurable performance gain.
