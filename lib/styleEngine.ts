...
export function buildStyle(config: PromptDNA): string {
  ...
  for (const p of rawParts) {
    if (p) {
      ...
      // Optimization: For small arrays (< 10 elements), a single-pass loop using .includes()
      // is significantly faster than allocating Set objects and creating intermediate arrays.
