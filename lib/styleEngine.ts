...
export function buildStyle(config: PromptDNA): string {
  ...
  // Optimization: For deduplicating very small arrays (e.g., under 10 items), avoid the object allocation overhead of `Array.from(new Set(array))`; using an inline `.includes()` check before adding items to the array is measurably faster.
