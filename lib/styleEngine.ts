...
export function buildStyle(config: PromptDNA): string {
  ...
  // Optimization: The parts array is strictly bounded to a max of 9 items.
  // For such small arrays, using an inline .includes() check (O(N^2)) is faster
  // than the overhead of allocating a new Set and converting back to an array.
