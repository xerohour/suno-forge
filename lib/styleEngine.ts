...
export function buildStyle(config: PromptDNA): string {
  ...
  for (const p of rawParts) {
    if (p) {
      ...
      // Optimization: For strictly bounded arrays (under 10 items), an O(N^2) inline
      // .includes() check is measurably faster than the object allocation overhead
      // of Array.from(new Set()).
