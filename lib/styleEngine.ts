...
export function buildStyle(config: PromptDNA): string {
  ...
  // Optimization: For strictly bounded, small arrays (like rawParts), avoiding Set allocation overhead
  // by using an inline .includes() check is faster. This is an O(N^2) operation, but N is tiny (~9 items).
