...
export function buildStyle(config: PromptDNA): string {
  ...
  // Optimization: For deduplicating small, strictly bounded arrays (under 10 items),
  // avoiding the object allocation overhead of Array.from(new Set()) by using an inline
  // .includes() check is measurably faster.
