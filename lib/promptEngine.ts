...
// Pre-compile regexes to avoid recompilation overhead in hot paths
const TIMESTAMP_CLEANUP_REGEX = /[-:T]/g;
const SAFE_TITLE_CLEANUP_REGEX = /[^a-z0-9_\s-]/g;
...
function generateTechnicalName(title: string): string {
    ...
    const safeTitle = title.toLowerCase().replace(SAFE_TITLE_CLEANUP_REGEX, ' ').trim().replace(WHITESPACE_REGEX, '_');
