...
const TIMESTAMP_CLEAN_REGEX = /[-:T]/g;
const SAFE_TITLE_INVALID_REGEX = /[^a-z0-9_\s-]/g;
const SAFE_TITLE_SPACE_REGEX = /\s+/g;

function generateTechnicalName(title: string): string {
    const now = new Date();
    const timestamp = now.toISOString().slice(0, 19).replace(TIMESTAMP_CLEAN_REGEX, ''); // YYYYMMDDHHMMSS
    const safeTitle = title.toLowerCase().replace(SAFE_TITLE_INVALID_REGEX, ' ').trim().replace(SAFE_TITLE_SPACE_REGEX, '_');
