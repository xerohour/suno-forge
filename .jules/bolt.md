...
## 2025-02-18 - Unnecessary Async Overhead
**Learning:** Marking purely synchronous functions with `async` introduces state machine and object allocation overhead (Promises) which degrades performance on hot paths.
**Action:** Remove `async` keywords and `Promise` return types from functions that don't perform asynchronous operations.
