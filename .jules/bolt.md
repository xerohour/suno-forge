...
## 2025-02-18 - Unnecessary Async Overhead in Synchronous Functions
**Learning:** Using `async` and returning `Promise` types for purely synchronous operations (like string concatenation and object construction in `buildPrompt`) introduces significant state machine and object allocation overhead (over 50% slower in microbenchmarks). In addition, if un-awaited in Next.js API routes, it results in serializing the Promise as an empty object `{"prompt": {}}`.
**Action:** Remove `async` and `Promise` return types from purely synchronous functions. Ensure all downstream call sites remove the corresponding `await` keywords or `Promise.all` wrappers.
