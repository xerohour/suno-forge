...
## 2025-02-18 - Fast deduplication for small arrays
**Learning:** For very small arrays (e.g. < 10 elements), a single-pass loop using Array.prototype.includes() is significantly faster than allocating Set objects and creating intermediate arrays, as it avoids memory allocation and iterator overhead.
**Action:** When filtering out duplicates from a small, fixed-size list of strings (like `parts` in `buildStyle`), use a manual `includes()` check inside the existing loop rather than instantiating a new `Set` and `Array.from`.
